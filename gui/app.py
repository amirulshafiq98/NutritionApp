...
class NutritionApp:
    ...
    def save_results(self):
        ...
        app_logger.info("Save operation requested.")
        if not self.last_patient_data or not self.last_calculated_results:

            # Prevent saving if no calculation has been performed yet
            messagebox.showwarning("Save Warning", "No results to save. Please calculate a nutrition plan first.")
            app_logger.warning("Save attempt failed: No results available.")
            return
        ...
        if file_path:
            ...
            self.results_panel.display_plan(self.last_patient_data, self.last_calculated_results)
